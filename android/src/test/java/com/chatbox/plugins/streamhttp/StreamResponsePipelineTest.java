...
public class StreamResponsePipelineTest {
    ...
    @Test
    public void endsAfterEmptyAcceptedResponse() throws Exception {
        TestHttpURLConnection connection = new TestHttpURLConnection(202, body(""), null);
        RecordingEvents events = new RecordingEvents();

        StreamResponsePipeline.consume("accepted-id", connection, () -> false, events);

        assertEquals(List.of("response", "end"), events.names);
        assertEquals(List.of(202), events.statuses);
        assertEquals(1, connection.inputStreamReadCount);
        assertEquals(0, connection.errorStreamReadCount);
    }
    ...
    public void pluginEventSinkNotifiesBridgeWithOrderedIdsAndPayloads() {
        ...
        assertTrue(events.onResponse("bridge-id", 429, new JSObject().put("content-type", "text/event-stream")));
        ...
        registry.cancel("bridge-id");
        assertFalse(events.onResponse("bridge-id", 200, new JSObject()));
    ...
    private static final class RecordingEvents implements StreamResponsePipeline.EventSink {
        ...
        @Override
        public boolean onResponse(String streamId, int status, JSObject headers) {
            names.add("response");
